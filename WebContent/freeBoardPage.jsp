				...
				<div class="col-sm-10">
					<div class="table">
							...
							<tbody>
								<% for(int i = 0; i < articleList.size(); i++){ %>
									<tr>
										...
										<td><a href="ShowDetailScript.do?boardType=F&num=<%= articleList.get(i).get("num") %>">
						...
						<% } else if(isAdmin.equals("0") || isAdmin.equals("1")){ %>
							<button type="submit" class="btn btn-warning" onClick="location.href='makeFreeScript.jsp'">글쓰기</button>
						<% } %>
						...
						<% if(nowPage <= 1){ %>
							[이전]
						<% } else{ %>
							<a href="ShowBoardList.do?boardType=F&page=<%= nowPage - 1 %>">[이전]</a>
						<% } %>
						...
						<% for(int i = startPage; i <= endPage; i++){ %>
							<% if(i == nowPage) { %>
								[<%= i %>]
							<% } else { %>
								<a href="ShowBoardList.do?boardType=F&page=<%= i %>">[<%= i %>]</a> &nbsp;
							<% } %>
						<% } %>
						...
						<% if(nowPage >= maxPage) { %>
							[다음]
						<% } else{ %>
							<a href="ShowBoardList.do?boardType=F&page=<%= nowPage + 1 %>">[다음]</a>
						<% } %>
