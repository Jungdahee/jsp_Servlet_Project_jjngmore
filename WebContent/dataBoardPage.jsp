				...
				<div class="col-sm-10">
					<div class="table">
							...
							<tbody>
								<% for(int i = 0; i < articleList.size(); i++){ %>
									<tr>
										...
										<td><a href="ShowDetailScript.do?boardType=D&num=<%= articleList.get(i).get("num") %>">
						...
						<% } else if(isAdmin.equals("0") || isAdmin.equals("1")){ %>
							<button type="submit" class="btn btn-warning" onClick="location.href='makeDataScript.jsp'">글쓰기</button>
						<% } %>
					...
					<section id="pageList" style="margin-left: 45%; margin-top: 2%;">
						...
						<% for(int i = startPage; i <= endPage; i++){ %>
							<% if(i == nowPage) { %>
								[<%= i %>]
							<% } else { %>
								<a href="ShowBoardList.do?boardType=D&page=<%= i %>">[<%= i %>]</a> &nbsp;
							<% } %>
						<% } %>
