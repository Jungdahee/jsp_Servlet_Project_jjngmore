				...
				<div class="col-sm-10">
					<div class="table1">
							...
							<tbody>
								<% for(int i = 0; i < articleList.size(); i++){ %>
									<tr>
										...
										<td><a href="ShowDetailScript.do?boardType=N&num=<%= articleList.get(i).get("num") %>">
						...
						<% } else if(isAdmin.equals("1")){ %>
							<button type="submit" class="btn btn-warning" onClick="location.href='makeNotifyPage.jsp'">글쓰기</button>
						<% } %>
						...
						<% if(nowPage <= 1){ %>
							[이전]
						<% } else{ %>
							<a href="ShowBoardList.do?boardType=N&page=<%= nowPage - 1 %>">[이전]</a>
						<% } %>
						...
						<% for(int i = startPage; i <= endPage; i++){ %>
							<% if(i == nowPage) { %>
								[<%= i %>]
							<% } else { %>
								<a href="ShowBoardList.do?boardType=N&page=<%= i %>">[<%= i %>]</a> &nbsp;
							<% } %>
						<% } %>
